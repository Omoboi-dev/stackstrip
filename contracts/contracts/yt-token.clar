;; yt-token: the Yield Token. Entitles the holder to the yield until maturity.
...
(define-data-var token-name (string-ascii 32) "Stackstrip Yield Token")
(define-data-var token-symbol (string-ascii 32) "YT")
...
(define-read-only (get-balance (who principal)) (ok (ft-get-balance yt who)))
(define-read-only (get-total-supply) (ok (ft-get-supply yt)))
